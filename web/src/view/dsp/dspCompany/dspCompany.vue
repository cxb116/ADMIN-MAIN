...
<template>
  ...
  <el-input v-model.number="searchInfo.dsp_code" placeholder="搜索条件" />
</el-form-item>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt" width="180">
            ...
            <el-table-column align="left" label="预算隐射" prop="dsp_code" width="120" />

            <el-table-column align="left" label="请求地址" prop="url" width="120" />

            <el-table-column align="left" label="请求方法" prop="method" width="120">
    <template #default="scope">
    {{ filterDict(scope.row.method,method_typeOptions) }}
    </template>
</el-table-column>
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateDspCompanyFunc(scope.row)">编辑</el-button>
    ...
    {{ detailForm.name }}
                    ...
                    <el-descriptions-item label="预算隐射">
    {{ detailForm.dsp_code }}
</el-descriptions-item>
...
const getTableData = async() => {
  const table = await getDspCompanyList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteDspCompanyByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateDspCompanyFunc = async(row) => {
    const res = await findDspCompany({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteDspCompanyFunc = async (row) => {
    const res = await deleteDspCompany({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'

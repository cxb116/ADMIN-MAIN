  ...
  <el-select v-model="searchInfo.dsp_company_id" placeholder="请选择公司" :clearable="true" filterable>
    ...
    {{ filterDict(scope.row.dsp_company_id, companyOptions) }}
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateDspProductFunc(scope.row)">编辑</el-button>
...
const getTableData = async() => {
  const table = await getDspProductList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteDspProductByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateDspProductFunc = async(row) => {
    const res = await findDspProduct({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteDspProductFunc = async (row) => {
    const res = await deleteDspProduct({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     ...
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return btnLoading.value = false
              ...
              switch (type.value) {
                case 'create':
                  res = await createDspProduct(formData.value)
                  break
                case 'update':
                  res = await updateDspProduct(formData.value)
                  break
                default:
                  res = await createDspProduct(formData.value)
                  break
